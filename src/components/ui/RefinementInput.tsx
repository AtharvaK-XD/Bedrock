    ...
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative group flex items-center" ref={dropdownRef}>
            <button
              ...
              className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 transition-colors rounded-xl px-3 py-1.5 border border-white/5 whitespace-nowrap text-xs font-mono"
            >
            ...
            {isModelDropdownOpen && (
              <div 
                className="absolute bottom-full right-0 mb-2 w-56 max-h-[300px] overflow-y-auto custom-scrollbar bg-[#1a1a1a]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-xl z-50 py-2"
                  ...
                  <div key={agent.id} className="mb-2">
                    ...
                    {agent.models.map((m) => (
                      <button
                        ...
                        className={cn(
                          "w-full flex items-center px-4 py-1.5 text-xs text-left transition-colors font-mono",
          ...
          <button 
            ...
            className="bg-copper-500 text-white px-4 py-1.5 rounded-xl hover:bg-copper-600 disabled:opacity-40 disabled:bg-white/10 disabled:text-gray-500 transition-all text-xs font-semibold uppercase tracking-wider"
          >
            Refine
          </button>
        </div>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2 flex-1 w-full justify-end">
          ...
          <span className="whitespace-nowrap text-right" title={timeUntilWeekly ? `Resets in ${timeUntilWeekly}` : undefined}>
            Weekly: {Math.round((weeklyTokens / WEEKLY_LIMIT) * 100)}%{timeUntilWeekly ? ` (${timeUntilWeekly})` : ''}
          </span>
        </div>
      </div>
    </form>
